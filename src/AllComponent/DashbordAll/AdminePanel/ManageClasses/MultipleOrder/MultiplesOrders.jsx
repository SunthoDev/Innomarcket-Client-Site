...
import "./multiplesOrder.css"
import { FaRegTrashAlt } from 'react-icons/fa';
import ManageClassCard from '../ManageClassCard/ManageClassCard';
import AuthProvider, { AuthContext } from '../../../../AuthoncationAll/AuthProvider/AuthProvider';
const MultiplesOrders = ({getChildData, orderCnt, orderData, handleDeleteUser, generateReport }) => {


let { _id, address, city, country, email, first, last, number, postcode, userEmail, userImage, userName,payment } = orderData;
let {setTestemail ,setToggleMultiData} = useContext(AuthContext)
        ...
        <tr>
            ...
            {(orderCnt == 1? 
             <button onClick={handleMoreDetails} className="DetailsClassButton">Details</button>
                 : 
                 <button onClick={handleSingleDetails} className="DetailsClassButton">History</button>
             )}
            ...
            <td>
                <button onClick={() => generateReport(_id, orderData)} className="deleteClassButton">Report</button>
            </td>
            {/* ------------------------------------------ */}
...
export default MultiplesOrders;